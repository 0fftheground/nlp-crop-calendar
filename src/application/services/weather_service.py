...
_WEATHER_REGION_PREFIX_PATTERNS = (
    re.compile(
        r"^(?P<region>[\u4e00-\u9fa5]{2,20}?)(?=(?:是)?(?:今天|明天|后天|昨天|前天|本周|这周|下周|下下周|最近|未来|从|20\d{2}年|\d{1,2}月|\d{1,2}[日号]))"
    ),
    re.compile(
        r"^(?!(?:今天|明天|后天|昨天|前天|本周|这周|下周|下下周|最近|未来))"
        r"(?P<region>[\u4e00-\u9fa5]{2,20}?)"
        r"(?=(?:是否|能否|能不能|可否|适合|适宜|可以).*(?:施肥|炼苗|移栽|翻地|打药|收割|整地))"
    ),
    re.compile(
        r"(?:20\d{2}[-/年]\d{1,2}[-/月]\d{1,2}[日号]?|\d{1,2}月\d{1,2}[日号]?)"
        r"(?:在)?(?P<region>[\u4e00-\u9fa5]{2,20}?)"
        r"(?=(?:哪天|何时|什么时候|是否|能否|能不能|可否|适合|适宜|可以|最适合))"
    ),
    re.compile(
        r"(?:今天|明天|后天|昨天|前天|本周|这周|下周|下下周|最近|未来)"
        r"(?:在)?(?P<region>[\u4e00-\u9fa5]{2,20}?)"
