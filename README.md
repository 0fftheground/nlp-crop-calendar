...
- **FastAPI Backend (`src/api/server.py`)** – exposes `/api/v1/handle` (auto route between tool & workflow) and `/api/v1/plan` (force LangGraph workflow).
- **Request Router (`src/agent/router.py`)** – LLM-powered router that decides whether to execute a standalone tool (`src/tools/registry.py`) or fall back to the LangGraph workflow.
- **LangGraph Workflow (`src/agent/workflows/crop_graph.py`)** – state machine with parsing, planning, and recommendation nodes backed by LangChain models.
- **Knowledge Base (`src/domain/knowledge_base.py`)** – curated crop-stage-task facts consumed by the planner.
...
## Development Notes
- `src/agent/router.py` + `src/agent/tool_selector.py` + `src/tools/registry.py` implement the LLM-based “tool vs. workflow” decision logic. Add tool handlers or adjust the routing prompt to expand coverage.
